...
// LEFT ==========================  
  ...
  else if (!strcmp(SolName, "UADJ")) { dirichlet = false; /*dirichlet = false;*/  }
...
// equation
    ...
    unsigned nDofsVadj = el->GetElementDofNumber(kel, solVadjType);    // number of solution element dofs
    unsigned nDofsPadj = el->GetElementDofNumber(kel, solPadjType);    // number of solution element dofs
...
//ADJ###################################################################
    ...
    for (unsigned i = 0; i < nDofsVadj; i++) {
      unsigned iNodeadj = el->GetMeshDof(kel, i, solVadjType);    // local to global solution node
      unsigned solVadjDof = msh->GetMetisDof(iNodeadj, solVadjType);    // global to global mapping between solution node and solution dof

      for (unsigned  k = 0; k < dim; k++) {
        solVadj[k][i] = (*sol->_Sol[solVadjIndex[k]])(solVadjDof);      // global extraction and local storage for the solution
        JACDof[i + k * nDofsVadj +nDofsVP] = pdeSys->GetKKDof(solVadjIndex[k], solVPdeadjIndex[k], iNodeadj);    // global to global mapping between solution node and pdeSys dof
      }
    }
    
    // pressure *************
    for (unsigned i = 0; i < nDofsPadj; i++) {
      unsigned iNodeadj = el->GetMeshDof(kel, i, solPadjType);    // local to global solution node
      unsigned solPadjDof = msh->GetMetisDof(iNodeadj, solPadjType);    // global to global mapping between solution node and solution dof
      solPadj[i] = (*sol->_Sol[solPadjIndex])(solPadjDof);      // global extraction and local storage for the solution
      JACDof[i + dim * nDofsVadj +nDofsVP] = pdeSys->GetKKDof(solPadjIndex, solPPdeadjIndex, iNodeadj);    // global to global mapping between solution node and pdeSys dof
    }
//ADJ###################################################################
...
//ADJOINT#############################################################################3	
        // *** get gauss point weight, test function and test function partial derivatives ***
        msh->_finiteElement[kelGeom][solVadjType]->Jacobian(coordX, ig, weight, phiVadj_gss, phiVadj_x_gss, phiVadj_xx_gss);
        phiPadj_gss = msh->_finiteElement[kelGeom][solPadjType]->GetPhi(ig);
...
//               NSV_gss[kdim]   +=  phiV_gss[i] * (solV_gss[jdim] * gradSolV_gss[kdim][jdim]);                                  //advection
	  ...
	  NSVadj_gss[kdim] += - solPadj_gss * phiVadj_x_gss[i * dim + kdim];
	    ...
	    aResVadj[kdim][i]   += /*- solVadj_gss[kdim]*phiVadj_gss[i]*weight; //*/ (/*alpha*(solV_gss[kdim]   - Vel_desired[kdim])*/ force[kdim] * phiVadj_gss[i] - NSVadj_gss[kdim]) * weight;
          }
        } // end phiV_i loop
	    ...
	    aResPadj[i] += /*-  solPadj_gss*phiPadj_gss[i]*weight;   //*/- (gradSolVadj_gss[kdim][kdim]) * phiPadj_gss[i]  * weight;
          }
        } // end phiP_i loop
...
//     unsigned nDofsVP_tot = 2*nDofsVP;
    ...
    unsigned nDofsVdes = el->GetElementDofNumber(kel, solVType);    // number of solution element dofs
...
//STATE###################################################################
  ...
  //DESIRED VEL###################################################################  
    ...
    for (unsigned i = 0; i < nDofsV; i++) {
      unsigned iNode = el->GetMeshDof(kel, i, solVType);    // local to global solution node
      unsigned solVdesDof = msh->GetMetisDof(iNode, solVType);    // global to global mapping between solution node and solution dof
...
//         phiP_gss = msh->_finiteElement[kelGeom][solPType]->GetPhi(ig);
	...
	msh->_finiteElement[kelGeom][solVType  /*solVdes*/]->Jacobian(coordX, ig, weight, phiVdes_gss, phiVdes_x_gss, phiVdes_xx_gss);
